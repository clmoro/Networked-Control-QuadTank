function [K,rho,feas]=LMI_DT_SpectAbs(F,G,H,N,ContStruc)
...
if abs(alpha)>=1
    disp('The constraint is not inside the unitary circle!')
    pause
end
...
LMIconstr = [[(1-(1-alpha)^2)*P-F*P*F'-F*L'*Gtot'-Gtot*L*F'-(1-alpha)*(P*F'+F*P+L'*Gtot'+Gtot*L),Gtot*L;
